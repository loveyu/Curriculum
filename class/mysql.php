...
class Mysql
{
	...
	*/
	function __construct(){
		$this->set_config();
		$mysql_con = mysql_connect($this->config['hostname'], $this->config['username'],
		...
		if(!mysql_select_db($this->config['database'],$mysql_con))
			$this->debug("无法选择数据库，或数据库".$db."不存在:",true);
	}
	...
	{
		if($this->config['db_debug'])echo "<div id=\"database_error\">",$title ? "<h2>$title</h2>" : "", mysql_error(),"</div>";
		if($close)exit;
	}
	private	function sql_to_arr($sql){
		$return=array();
		$result = $this->query($sql);
		if(!$result)$this->debug("数据选择失败");
		$i=0;
